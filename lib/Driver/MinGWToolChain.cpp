...
MinGW::MinGW(const Driver &D, const llvm::Triple &Triple, const ArgList &Args)
  ...
  Arch += "-w64-mingw32";
  std::error_code EC;
  llvm::sys::fs::directory_iterator MingW64Entry(LibDir.str() + Arch, EC);
  if (!EC) {
    GccLibDir = MingW64Entry->path();
  } else {
    // If mingw-w64 not found, try looking for mingw.org.
    Arch = "mingw32";
    llvm::sys::fs::directory_iterator MingwOrgEntry(LibDir.str() + Arch, EC);
    if (!EC)
...
Tool *MinGW::buildLinker() const { return new tools::MinGW::Link(*this); }
